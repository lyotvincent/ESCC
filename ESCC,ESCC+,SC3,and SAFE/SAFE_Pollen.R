for( tot in 1:100)
{
  ...
  matrix_filtered=exp(data[which(gene_filter),1:301])
  ...
  cluster.ensemble <- SAFE(cluster_results = cluster.results, program.dir = "gpmetis_and_shmetis_for_Linux", k_min=11,k_max=11,
                           MCLA = TRUE, CSPA = TRUE, HGPA = TRUE, SEED = 123)
  library(cidr)
  cat(cluster.ensemble$optimal_clustering,file="SAFE_Pollen.txt",append=TRUE)
  cat("\n",file="SAFE_Pollen.txt",append=TRUE)
}
for( tot in 1:100)
{
  ...
  library('tidyverse')
  a<-read_rds('path for goolam.rds')
  print(a)
  ...
  cluster.ensemble <- SAFE(cluster_results = cluster.results, program.dir = "path for gpmetis_and_shmetis_for_Linux", k_min=5,k_max=5,MCLA = TRUE, CSPA = TRUE, HGPA = TRUE, SEED = 123)
  library(cidr)

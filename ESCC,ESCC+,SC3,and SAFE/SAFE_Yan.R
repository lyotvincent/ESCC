for( tot in 1:27)
{
  ...
  matrix_filtered=exp(data[which(gene_filter),1:90])
  ...
  cluster.ensemble <- SAFE(cluster_results = cluster.results, program.dir = "gpmetis_and_shmetis_for_Linux", k_min=7,k_max=7,
                           MCLA = TRUE, CSPA = TRUE, HGPA = TRUE, SEED = 123)
  library(cidr)
  cat(cluster.ensemble$optimal_clustering,file="SAFE_Yan.txt",append=TRUE)
  cat("\n",file="SAFE_Yan.txt",append=TRUE)
}